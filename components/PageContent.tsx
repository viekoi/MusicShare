...
const PageContent:React.FC<PageContentProps> = ({songs}) => {
  ...
  return (
    <div className="list xl:gap-0 lg:gap-1 md:gap-2 gap-3">
      ...
      {songs.map((song) => {
        return (
          <SongItem onClick={(id: string) => onPlay(id)}  key={song.id} id={song.id} userId={song.user_id} author={song.author} title={song.title} songPath={song.song_path} imagePath={song.image_path} />
        )
      })}
